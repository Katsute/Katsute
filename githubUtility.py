...
def get_statistics(user, private, hide_languages, max_languages, hide_repos):
    int_user_id = user.id
    ...
    for repo in user.get_repos():
        if repo.id in hide_repos or (repo.private and not private):
            continue
        ...
        # issues
        map_stats['issues'] += repo.get_issues(since=date_init_year, state="all", creator=user.name).totalCount
        # pulls
        for pull in repo.get_pulls(state="all"):
            if pull.user.id == user.id and pull.updated_at >= date_init_year:
                map_stats['pulls'] += 1
        # commits
        map_stats['commits'] += repo.get_commits(since=date_init_year, author=user.name).totalCount

        # language statistics
        double_contrib = repo.get_commits(author=user.name).totalCount / repo.get_commits().totalCount  # what % of commits is made by this user
        for lang, count in repo.get_languages().items():
            if not (lang in hide_languages):  # remove hidden config
